from ._forward import ForwardPasser, Node
...
class Earth(BaseEstimator, RegressorMixin, TransformerMixin, MultiOutputMixin):
    ...
    def __init__(self, max_terms=None, max_degree=None, allow_missing=False,
                 ...
                 feature_importance_type=None, allow_subset = True, verbose=0):
    ...
    def fit(self, X, y=None,
        ...
        self.fill_with_Earth(self.Node)
        self.Node.n = X.shape[0]

        if self.Node.forward_passer == None:
            raise ValueError("forward_pass should not have returned 'None'")
        
        # Error: This is unneccesary???
        self.fit_tree(self.Node, X, y, sample_weight, output_weight, missing)
        if self.enable_pruning is True:
            pruned_node = self.pruning_pass(X, y, sample_weight, output_weight, missing,skip_scrub=True)
            self.Node = pruned_node
            self.fit_tree(self.Node, X, y, sample_weight, output_weight, missing)
...
#
    ...
    def fill_with_Earth(self, Node):
            # Take in the node with a forward_passer and fill it with an Earth model
            model = Earth()
            model.basis_ = Node.forward_passer.get_basis()
            model.forward_pass_record_ = Node.forward_passer.trace()
            Node.Earth = model
            ...
            if Node.left_child == None and Node.right_child == None:
                ...
                self.fill_with_Earth(Node.left_child)
                self.fill_with_Earth(Node.right_child)


    def fit_tree(self, Node, X, y, sample_weight, output_weight, missing):
        if hasattr(Node.Earth, 'smooth') and Node.Earth.smooth:
            #Error: Why does this never happen?
            print("Smoothing")
            Node.Earth.basis_ = Node.Earth.basis_.smooth(X)
        Node.Earth.linear_fit(X, y, sample_weight, output_weight, missing, skip_scrub = True)

        Node.node_GCV = Node.Earth.gcv_

        # Remove the forward passer thing so it can be copied pog??
        Node.forward_passer = None

        #Recursion
        #Error: Don't technically need to keep subsetting like this??
        if Node.left_child == None and Node.right_child == None:
            return
        else:
            left_subset = X[:, int(Node.split_var)] <= Node.split_val
            self.fit_tree(Node.left_child,  X[left_subset], 
                                            y[left_subset],
                                            sample_weight[left_subset], 
                                            output_weight, 
                                            missing[left_subset])
            right_subset = X[:, int(Node.split_var)] >  Node.split_val
            self.fit_tree(Node.right_child, X[right_subset], 
                                            y[right_subset],
                                            sample_weight[right_subset], 
                                            output_weight, 
                                            missing[right_subset])
    ...
    def forward_pass(self, X, y=None,
        ...
        forward_passer = ForwardPasser(
            ...
            xlabels=self.xlabels_, linvars=linvars, allow_subset = self.allow_subset, **args)
        # Create the node and run the forward pass
        self.Node = Node()
        self.Node.X = X
        self.Node.y = y
        self.Node.sample_weight = sample_weight
        self.Node.missing = missing
        forward_passer.set_node(self.Node)
        self.Node.set_forward_passer(forward_passer)
        self.Node.run_forward_pass()
...
#             sample_weight = np.repeat(sample_weight,y.shape[1],axis=1)
        ...
        pruning_passer = PruningPasser(
            ...
            **args)
        #pruning_passer.run()
        pruned_node = pruning_passer.node_prune(self.Node)
...
#         self.pruning_pass_record_ = record
    ...
    def summary(self):
        self.summary_help(self.Node, "")

    def summary_help(self, Node, current_subset):
        '''Goes through the tree and prints an individual summary for each model'''
        if Node.left_child == None and Node.right_child == None:
            print("Model on: ", current_subset)
            print(Node.Earth.summary_individual(), "\n")
            return

        self.summary_help(Node.left_child, current_subset + "\nVar: " + str(Node.split_var) +  " Less than: " + str(Node.split_val))
        self.summary_help(Node.right_child, current_subset + "\nVar: " + str(Node.split_var) + " More than: " + str(Node.split_val))


    def summary_individual(self):
...
#             sample_weight = np.repeat(sample_weight,y.shape[1],axis=1)
        ...
        #** Error: I added this thing because sometimes GCV was infinity. If would be, add to the penalty. 
        if (coef.shape[0] + self.get_penalty() * (coef.shape[0]-1)/2.0) == X.shape[0]:
            self.gcv_ = gcv(self.mse_,
                            coef.shape[0], X.shape[0],
                            self.get_penalty() + 1)
            print("Making self.gcv: ", self.gcv_)
            print("Would have been: ", gcv(self.mse_, coef.shape[0], X.shape[0], self.get_penalty()))
        else:
            self.gcv_ = gcv(self.mse_,
                            coef.shape[0], X.shape[0],
                            self.get_penalty())
...
#         self.grsq_ = ((1 - (gcv_ / gcv0)) * output_weight).sum()
    ...
    def predict(self, X, missing=None, skip_scrub=False):
       ...
       -------
        ...
        # Remove names and stuff
        if not skip_scrub:
            X, missing = self._scrub_x(X, missing) #Is this output correct?
        y_prediction = np.empty(shape = (X.shape[0], 1))

        for i in range(0, len(X)):
            y_prediction[i] = self.predict_help(self.Node, X[i:i+1])
        return y_prediction[:, 0]
        

    def predict_help(self, Node, X):
        # Takes in one datapoint and makes one prediction
        # Error: Is this slow??? 
        # Yes this is slow because dot products are faster than a bunch of individual stuff I think
        # Using full matrices tho requires indexes on the numpy array. Sounds annoying
        # This is where the instructions matrix might be helpful!
        if Node.left_child == None and Node.right_child == None:
            # Error: Is all this neccesary for just one prediction??
            B = Node.Earth.transform(X)
            y = np.dot(B, Node.Earth.coef_.T)
            return y
        
        if X[:, int(Node.split_var)] <= Node.split_val:
            return self.predict_help(Node.left_child, X)
        elif X[:, int(Node.split_var)] > Node.split_val:
            return self.predict_help(Node.right_child, X)
        else:
            ValueError("Should not have got here. Failed to predict")
